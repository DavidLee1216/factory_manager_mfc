...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	else if (nChar >= '0' && nChar <= '9')
	{
		...
		pCell->SendMessage(WM_KEYDOWN, nChar, 0);
...
BOOL CGridCtrl::SetCellType(int nRow, int nCol, CRuntimeClass* pRuntimeClass)
{
	...
	pNewCell->Create(NULL, NULL, WS_VISIBLE, CRect(0, 0, 0, 0), this, 1000);
...
int CGridCtrl::GetGridHeight(int rowLimit)
{
	int fixedHeight = 0;// GetFixedRowHeight();
	CCellRange VisCellRange = GetCellRange();
	int maxVisibleRow = VisCellRange.GetMaxRow();
	int minVisibleRow = VisCellRange.GetMinRow();
	int height = fixedHeight;
	int i = 0;
	int minRow = min(rowLimit, maxVisibleRow);
	if (rowLimit == 0)
		minRow = maxVisibleRow;
	for (i = minVisibleRow; i <= minRow; i++)
	{
...
void CGridCtrl::SetItemInt(int nRow, int nCol, int nVal)
{
    TCHAR temp[100];
    _stprintf_s(temp, _T("%d"), nVal);
    SetItemText(nRow, nCol, temp);
}


void CGridCtrl::SetItemDouble(int nRow, int nCol, double dblVal)
{
	TCHAR temp[100];
	_stprintf_s(temp, _T("%.2f"), dblVal);
	SetItemText(nRow, nCol, temp);
}
